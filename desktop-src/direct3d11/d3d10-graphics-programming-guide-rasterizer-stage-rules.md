...
Any pixel center which falls inside a triangle is drawn; a pixel is assumed to be inside if it passes the top-left rule. The top-left rule is that a pixel center is defined to lie inside of a triangle if it lies on a left edge while not lying on the bottom edge or if it lies on the top edge while not lying on a right edge.
...
-   A right edge, is an edge that is not exactly horizontal and is on the right side of the triangle. A triangle can have one or two left edges.
